 ...
 * ─────────────────────────────────────────────────────────────────────
 * 我们要用 **Electron 自带的 Node** 跑这个 npm，而两者的 `engines.node` 可能对不上。
 * 当初钉这个版本时外壳带的是 Electron 37.10.3 / **Node 22.21.1**：
 *
 ...
 *
 * ★ 2026-09-25 晚些时候 Electron 升到了 **44.0.0（Node 24.21.0）** —— 11.20.0 在那边
 *   同样满足，所以这个钉子**不需要动**。要不要趁机升到 12.x 是另一回事
 *   （见 ⑦.5：那条判据会去问**当前的 Electron 本体**，不会因为换 Electron 而失准）。
 *
 ...
 *
 * ★ 2026-09-25 晚些时候 Electron 升到 **44.0.0**（Node 24.21.0）—— 这条判据**自动跟着走**
 *   （下面它去问的是**当前的 Electron 本体**，不是写死的版本号）。这就是当初这么写的好处：
 *   换 Electron 时这里不需要改一行代码，`npm 11.20.0` 在新 Node 上照样满足。
 *
...
function enginesVerdict(npmDir) {
  ...
  //   两者版本**不一样**：2026-09-25 那天实测，系统 node = 24.20.0，
  //   而当时的 Electron 37.10.3 带的是 22.21.1（后来升到 44.0.0 / Node 24.x，仍然不一样）。
