...
### ★ 装之前：内核不用你操心（0.2.14 起）

**客户端只装外壳、不带内核**（刻意的，这样内核能独立升级）。
但**内核也不需要你自己装** —— 0.2.14 起外壳会替你装：

* 第一次打开客户端时，如果这台电脑上**没有**内核，它会**自己**从官方渠道
  （`registry.npmjs.org`）装一份到**用户数据目录**（`<userData>\kernel\<版本>\`）；
* **不需要你装 Node.js** —— 外壳用 Electron 自带的 Node 跑随包的 npm；
* **已经有内核就不会重复下载**（一个字节都不下），也不会动你已有的任何东西。

三个入口，随便哪个都行：

| 情况 | 入口 |
|---|---|
| 全新机器，客户端起不来 | 加载页会**自动拉开**「诊断与修复」→ 点「**下载并安装内核**」 |
| 已经进了界面 | **外壳设置 → 更新 → 运行环境** → 勾选后点「开始安装」 |
| 想自己来 | `npm i -g @deepseek-ai/dsh`（要本机有 Node.js） |
...
每一条都是拿那一版内核**真启动一次**验出来的，不是推断：

| 内核版本 | 渠道 | 在这个客户端（Electron 44.0.0）上 |
|---|---|---|
| `0.1.5-rc.2` | （原来在用的） | ✅ |
| `0.1.5-rc.3` | `latest`（正式） | ✅ |
| `0.1.7-rc.2` | `next`（预览） | ✅ |
| `0.1.7-alpha.2` | `alpha`（实验） | ✅ |

> **卡点曾经是 Electron 版本。** `0.1.6` 起内核新增了运行时拦截，要 hook V8 内部去
> `require` 内置模块，而加载器校验的是 **V8 的精确指纹**——只认
> `43.0.0 / 44.0.0 / 45.0.0-alpha.6`。0.2.15 把外壳的 Electron 从 **37.10.3 升到 44.0.0**，
> 0.1.6+ 内核因此可用（升之前那四个版本里 0.1.7 的两个都是 `Unsupported/no-context`）。
>
> ★ **`electron` 在 `package.json` 里是精确钉死的（`44.0.0`，不带 `^`）** ——
> 「顺手升到最新」在这里**是错的**：实测 **Electron 44.4.5 照样起不来**。
> 要换 Electron，先 `node scripts/kernel-compat-check.js --version=<内核版本> --electron=<新目录>` 真跑一次。
>
> ★ 另一条容易误解的：`npm i -g @deepseek-ai/dsh` **只装 `latest`** —— 命令行装不到 0.1.7。
> 界面会写清这一点，装别的渠道只能用**下载下来的那个 `.tgz`**。
>
...
或把内核放到 `<项目>/vendor/dsh/`，或让客户端自己装
（设置 → 更新 → 运行环境 → 开始安装；见上文「内核不用你操心」）。
