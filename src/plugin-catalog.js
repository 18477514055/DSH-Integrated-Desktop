 ...
 *
 * 为什么要有这个：hub 里现在有 `dsh-plugin-uploader`（插件上传器）——
 * 那是给自己用的**生产工具**，不是给终端用户的插件。普通用户勾了它没有任何用处。
 *
 * ★ 这只是**过渡**：正经做法是索引里带一个 `tier` / `hidden` 字段。
 *   所以下面先读索引字段（`tier` / `hidden` / `keywords` 里的 `dev`），
 *   读不到才退回这张写死的名单 —— **生产端一旦补上字段，外壳不用改就能生效**。
 */
const DEV_ONLY_FALLBACK = new Set(["dsh-plugin-uploader"]);
